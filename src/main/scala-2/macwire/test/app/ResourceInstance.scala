...
class DBResourceA(implicit named: Named) {
  def resource[F[_]: Monad]: Resource[F, DBResource] = Applicative[Resource[F, *]].pure(new DBResource {
...
class PrintStringResource(implicit named: Named) {
  def resource[F[_]: Monad]: Resource[F, PrintString] = Applicative[Resource[F, *]].pure(new PrintString(named.appName))
}
